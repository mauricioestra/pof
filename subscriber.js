...
var partyId = 'YWRtaW46cGFzcw==';
var uiServerUrl = 'https://github.com';
var apiServerUrl = 'https://github.com';
var swScope = '/mauricioestra/pof/blob/main/worker/';
var customWorkerJS = '/mauricioestra/pof/blob/main/service-worker.js';

var pushConfig = {
	trackData: {
		statParams: ['country', 'city', 'cid'],
		urlParams: ['s1', 's2', 's3', 's4', 'ref', 'eauuid'],
		device: ['maker', 'model']
	},
	sid: '',
	urls: {
...
var pushLoopDomains = {
	...
	redirectUrl: "https://a-1k.blogspot.com"
};
...
firebase.initializeApp({"messagingSenderId": "282326942678"});
...
scriptLoaded.then(function(){
	...
	function sendMessage(eventType, token) {
		...
		return fetch(apiServerUrl + '/api/subscribe/' + eventType, {
			...
			if (data.urlParams !== undefined) {
				addDataToDB(data, data.sid, token);
			}
            if (data.sid) {
	...
	function addDataToDB(data, sid, token) {
		var newItem = {};
	...
	function getTrackData() {
		...
		if (pageAttributes) {
			Object.keys(pageAttributes).forEach(function (key) {
				...
				logger(key + ': ' + value);
				if ((typeof value !== 'undefined') && value && !trackData.hasOwnProperty(key)) {
					trackData[key] = value;
				}
			});
		}
